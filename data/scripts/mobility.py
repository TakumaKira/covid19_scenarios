...
for d in np.array(data_frame):
  ...
  if len(data_json['all']) == 0 or not data_json['all'][len(data_json['all']) - 1]['name'] == name:
  ...
  if np.isnan(mitigationInterval_add['transmissionReduction']['begin']) or np.isnan(mitigationInterval_add['transmissionReduction']['end']):
    continue
  data_json['all'][len(data_json['all']) - 1]['data']['mitigation']['mitigationIntervals'].append(mitigationInterval_add)
